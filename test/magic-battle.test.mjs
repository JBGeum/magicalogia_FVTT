...
describe("buildBoostCard", () => {
  ...
  it("집중방어 focus 배열: dice [4,4,2], focus [4] → 눈4 전부 cancel(무제한), 2 valid", () => {
    const c = buildBoostCard({ who: "x", n: 3, dice: [4, 4, 2], struck: [], focus: [4] });
    expect(c.dice.map((d) => d.st)).toEqual(["cancel", "cancel", "valid"]);
  });

  it("집중 2-눈 focus 배열: dice [2,4,5], focus [2,4] → 2·4 cancel, 5 valid", () => {
    const c = buildBoostCard({ who: "x", n: 3, dice: [2, 4, 5], struck: [], focus: [2, 4] });
    expect(c.dice.map((d) => d.st)).toEqual(["cancel", "cancel", "valid"]);
  });

  it("focus 기본 [] / null 허용: dice [3], struck [] → valid", () => {
    expect(buildBoostCard({ who: "x", dice: [3] }).dice[0].st).toBe("valid");
    expect(buildBoostCard({ who: "x", dice: [3], focus: null }).dice[0].st).toBe("valid");
  });
